...
function dockerIP() { 
	IP=`docker inspect geth_stunnel_rinkeby|grep -w IPAddress -m1|grep -oE [0-9.]+`;
	ssh -L 8545:localhost:8545 -N -f -l eleven ${IP};
}

function dockerInit() {
	docker start geth_stunnel_rinkeby 2> /dev/null || \
	docker run --name geth_stunnel_rinkeby -p 30303:30303 -p 30303:30303/udp -p 30304:30304/udp -td -v"${DATADIR}:/data:z" geth_stunnel_rinkeby
}
